...
import static hassan.docappoint.R.attr.layoutManager;

public class DocAppoinment extends AppCompatActivity {
    ...
    ArrayList<AppoinmentDetails> appDetails;


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_doc_appoinment);
    ...
    private void requestApi(JSONObject data, String reqUrl, String method) {
        ...
        JsonArrayRequest request = new JsonArrayRequest(reqMethod, reqUrl, data, new Response.Listener<JSONArray>() {
            ...
            public void onResponse(JSONArray response) {
                ...
                Toast.makeText(DocAppoinment.this, response.toString(), Toast.LENGTH_LONG).show();
                parseData(response);
            }
    ...
    private void parseData(JSONArray response) {
        ...
        try{
            for(int i = 0; i < response.length(); i++){
                ...
                appDetails.add(new AppoinmentDetails(data.getString("doctorName"), data.getString("date")));
            }
